...
def stage1_train(args: Config, device, resume: str, epochs1: int, outdir: str, val_every: int,
    ...
    best_any_path = os.path.join(outdir, "best_model")
    best_stage1_clean = os.path.join(outdir, f"best_stage1_fp32__{tag}.pth")
    final_clean = os.path.join(outdir, f"stage1_fp32__{tag}.pth")
    last_loss = None
...
def stage2_qat(args: Config, device, resume_from: str, resume_qat: str, epochs2: int, outdir: str, val_every: int,
    ...
    tag = fmt_ckpt_tag(args)
    best_any_path = os.path.join(outdir, "best_model")
    best_qat_clean = os.path.join(outdir, f"best_stage2_qat__{tag}.pth")
    qat_final_clean = os.path.join(outdir, f"stage2_qat__{tag}.pth")
    last_loss = None
    ...
    freeze_weights(infer_model, args)
...
def main():
    ...
    # ---- NEW: test precision & throughput flags ----
    parser.add_argument("--test_precision", type=str, default="fp32",
                        choices=["fp32", "fp16", "bf16", "int8", "e5m2", "e4m3"],
                        help="Precision used during test/eval only.")
    parser.add_argument("--measure_tp", action="store_true",
                        help="Measure GPU-only throughput during test (exclude data loading/memcpy).")
    parser.add_argument("--warmup", type=int, default=10,
                        help="Number of warmup batches ignored in throughput.")
    parser.add_argument("--tp_include_loss", action="store_true",
    ...
    if args_cli.enable_p1:
        p1_ckpt = stage1_train(
            ...
            test_precision=args_cli.test_precision,
